...
and cmp_stmt (tc : TypeCtxt.t) (c:Ctxt.t) (rt:Ll.ty) (stmt:Ast.stmt node) : Ctxt.t * stream =
  ...
  *)
  | Ast.Cast (typ, id, exp, notnull, null) ->
     ...
     let ll_refty = cmp_ty tc (TRef typ) in
     let notnull_code = cmp_block tc (Ctxt.add c id (Ptr ll_refty, Id id)) rt notnull in
     ...
     c, exp_code
        ...
        >:: T (Cbr (Id tmp, lt, le))
        >:: L lt
          >:: I (id, Alloca ll_refty)
          >:: I ("", Store (ll_refty, exp_op, Id id))
          >@ notnull_code
          >:: T(Br lm)
        >:: L le
          >@ null_code
