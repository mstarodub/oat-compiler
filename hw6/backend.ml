...
*)
(* Model a graph as a list of uids and neighbour lists *)
type node = (uid * (uid list))
type graph = node list
type color = int
type coloring = {
    colored_nodes : (uid * color) list
  ; spilled_nodes : uid list
}
let print_node ((uid, adj):node) : string =
  String.concat " : " [uid; String.concat " " adj]

let print_colored_node ((uid, color):(uid * color)) : string =
  String.concat " " [uid; string_of_int color]

let print_colored_nodes (l:(uid * color) list) : string =
  String.concat "\n" (List.map print_colored_node l)

let string_of_int_list (l: int list) : string =
  String.concat " " (List.map string_of_int l)

let print_graph (g:graph) : string =
  String.concat "\n" (List.map print_node g)

let cons_uniq (x: 'a) (xs: 'a list) : 'a list =
  if List.exists ((=) x) xs then xs else x::xs

(* remove an element from a list *)
let remove (a: 'a) (l: 'a list) : 'a list =
  List.filter ((<>) a) l

(* Add an edge to a graph *)
let add_edge (g:graph) ((n,m):uid * uid) : graph =
  if n = m then failwith "cannot create edge to self";
  if None = List.assoc_opt n g then failwith "node not in graph";
  if None = List.assoc_opt m g then failwith "node not in graph";
  let n_adj = cons_uniq m (List.assoc n g) in
  let m_adj = cons_uniq n (List.assoc m g) in
  let g' = g |> List.remove_assoc n |> List.remove_assoc m in
  (n, n_adj)::(m, m_adj)::g'

(* remove neighbour from node *)
let remove_neighbour_from_node (uid: uid) ((uid2, adj): node) : node =
  (uid2, remove uid adj)

(* Remove a node from a graph *)
let remove_node (g:graph) (uid:uid) : graph =
  (* Remove node from graph *)
  let g' = List.remove_assoc uid g in
  (* Remove node from adjacency lists *)
  List.map (remove_neighbour_from_node uid) g'


(* Return list of all pairs of elements in list *)
let rec all_pairs (l: 'a list) : ('a * 'a) list =
  let pair = fun x y -> (x, y) in
  match l with
    | [] -> []
    | x::xs -> (List.map (pair x) xs) @ (all_pairs xs)

(* Add edges between all edges of a list of nodes *)
let add_all_edges (g:graph) (l:uid list) : graph =
  List.fold_left add_edge g (all_pairs l)

let has_less_than_k_neighbours (k: int) ((uid, adj): node) : bool =
  (List.length adj) < k

(* remove all elements of one list from another *)
let rec remove_all (xs: 'a list) (ys: 'a list) : 'a list =
  match ys with
    | [] -> []
    | ys -> begin match xs with
      | [] -> ys
      | x::xs -> remove_all xs (remove x ys)
    end

(* 
    Coloring algorithm:
      1. Find node with <k neighbours and remove it
      2. Recursively k-color the remaining graph
      3. There must be a color remaining to color the node

      If there is no node with <k neighbours, spill a node
*)
let rec k_color_graph (g:graph) (k:int) : coloring =
  (* find a node with less than k neighbours *)
  let find_node (g:graph) (k:int) : node option =
    let node_opt = try Some (List.find (has_less_than_k_neighbours k) g)
      with Not_found -> None
    in
    node_opt
  in

  (* Assume graph has less than two nodes *)
  let base_cases (g:graph) (k: int) : coloring =
    if k = 0 then
      { colored_nodes = [];
        spilled_nodes = List.map fst g
      }
    else if List.length g = 0 then
      { colored_nodes = [];
        spilled_nodes = []
      }
    else if List.length g = 1 then
      { colored_nodes = [fst (List.hd g), 0];
        spilled_nodes = []
      }
    else failwith "not a base case!"
  in
  ...
  let normal_case (g:graph) (k:int) : coloring =
    match find_node g k with
      | None -> spill_node g k
      | Some (uid, adj) ->
        let subgraph = remove_node g uid in
        let {colored_nodes; spilled_nodes} = k_color_graph subgraph k in
        
        let adj_colors = List.filter (fun (uid, color) -> List.exists ((=) uid) adj) colored_nodes in
        let used_colors = List.map snd adj_colors in
        
        let available_colors = List.init k Fun.id in
        let remaining_colors = remove_all used_colors available_colors in

        if List.length remaining_colors = 0 then failwith "broken invariant - no color remaining";
        let color = List.hd remaining_colors in
        {
          colored_nodes = (uid, color)::colored_nodes;
          spilled_nodes = spilled_nodes
        }
  in
...
let better_layout (f:Ll.fdecl) ({live_in; live_out}:liveness) : layout =
  ...
  let (assn_uids, insn_uids, other_allocs) = fold_fdecl
    ...
    (fun (assn, insn, other) (x, i) -> 
      if insn_assigns i
      then (x::assn, x::insn, other)
      else (assn, x::insn, (x, Alloc.LVoid)::other))
    (fun a _ -> a)
    ([],[],[]) f
  in
